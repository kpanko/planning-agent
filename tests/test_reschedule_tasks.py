...
from todoist_mcp.server import TaskReschedule, reschedule_tasks
...
def mock_reschedule(monkeypatch):
    fn = MagicMock()
    monkeypatch.setattr(server, "_reschedule_task", fn)
    return fn
...
def test_date_only_calls_reschedule(mock_api, mock_reschedule):
    ...
    result = reschedule_tasks([TaskReschedule(task_id="1", date="2026-03-10")])
...
def test_date_shortcuts_today_tomorrow(mock_api, mock_reschedule):
    ...
    result = reschedule_tasks([
        TaskReschedule(task_id="1", date="today"),
        TaskReschedule(task_id="2", date="tomorrow"),
    ])
...
def test_with_time_calls_update_task(mock_api, mock_reschedule):
    ...
    result = reschedule_tasks([
        TaskReschedule(task_id="1", date="2026-03-10", time="09:30")
    ])
...
def test_time_none_does_not_call_update_task(mock_api, mock_reschedule):
    ...
    reschedule_tasks([TaskReschedule(task_id="1", date="2026-03-10", time=None)])
...
def test_multiple_tasks_mixed(mock_api, mock_reschedule):
    ...
    result = reschedule_tasks([
        TaskReschedule(task_id="1", date="2026-03-10", time="14:00"),
        TaskReschedule(task_id="2", date="2026-03-11"),
    ])
...
def test_error_recorded_per_task(mock_api, mock_reschedule):
    ...
    result = reschedule_tasks([
        TaskReschedule(task_id="1", date="2026-03-10"),
        TaskReschedule(task_id="2", date="2026-03-10"),
    ])
