...
- `GOOGLE_CALENDAR_ID` — Calendar ID of the calendar the agent
  reads for context (e.g. `abc123@group.calendar.google.com`).
  Find it in Google Calendar → Settings for that calendar →
  "Integrate calendar". Use a dedicated calendar that holds only
  events you consider hard blocks (appointments, things you don't
  want scheduled over) — not your primary calendar. If unset, the
  agent's calendar context block shows `(GOOGLE_CALENDAR_ID not
  set)` and no calendar data reaches the agent; there is no silent
  fallback to the primary calendar.
